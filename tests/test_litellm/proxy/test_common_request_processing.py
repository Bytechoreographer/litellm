...
@pytest.mark.asyncio
async def test_disconnect_cleanup_bills_partial_usage():
    """A client disconnect now records partial spend via _record_partial_usage_for_failure + failure handler."""
    logging_obj = MagicMock()
    ...
    response._record_partial_usage_for_failure = MagicMock()
    ...
    response._record_partial_usage_for_failure.assert_called_once()
    logging_obj.async_failure_handler.assert_awaited_once()


@pytest.mark.asyncio
async def test_disconnect_cleanup_skips_billing_when_no_chunks():
    """If no chunks were received, nothing to bill."""
    logging_obj = MagicMock()
    logging_obj.model_call_details = {}
    logging_obj._on_deferred_stream_complete = None
    logging_obj._deferred_stream_complete_args = None
    ...
    response._record_partial_usage_for_failure.assert_not_called()
    logging_obj.async_failure_handler.assert_not_awaited()


@pytest.mark.asyncio
async def test_disconnect_cleanup_skips_billing_when_already_dispatched():
    """If normal completion already logged success, do not double-log on disconnect."""
    logging_obj = MagicMock()
